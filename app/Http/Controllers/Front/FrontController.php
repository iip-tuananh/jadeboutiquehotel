...
use App\Model\Admin\BannerPage;
...
class FrontController extends Controller
{
    ...
    public function homePage() {
        ...
        $data['about'] = About::query()->with(['image', 'galleries' => function ($q) {
                $q->select(['id', 'about_id', 'sort'])
                    ->with(['image'])
                    ->orderBy('sort', 'ASC');
            },
        ])->first();

        $data['categoriesSpecial'] = CategorySpecial::query()->with(['rooms.image'])
            ->has('rooms')
            ->where('show_home_page', 1)
            ->orderBy('order_number')->get();

        $data['services'] = Service::query()->with(['image'])
            ->where('show_home_page', 1)
    ...
    public function getListRooms() {
        $data['rooms'] = Room::query()->with(['image'])->latest()->paginate(15);

        return view('site.room_category', $data);
    }

    public function about_page(Request $request) {
        $banner = BannerPage::query()->with('image')
            ->where('page', 'contact-banner')->first();

        return view('site.about_page', compact( 'banner'));
    }

    public function services(Request $request, $slug = null) {
       $categoryService = PostCategory::query()->with('image')->where('slug', $slug)->first();
       $services = Service::query()->with(['image', 'galleries.image'])->where('cate_id', $categoryService->id)
           ->where('status', 1)
           ->latest()->get();
    ...
    public function getServiceDetail(Request $request, $slug) {
        $service = Service::query()->with(['category', 'image', 'galleries.image'])->where('slug', $slug)->first();
    ...
    public function blogDetail(Request $request, $slug) {
        $blog = Post::query()->with(['image', 'category'])->where('slug', $slug)->first();
